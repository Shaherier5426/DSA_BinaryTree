...
while(pointer != null){
    parent = pointer;
    if(pointer.getData().getStudentLastName().toLowerCase().compareTo(data.getStudentLastName().toLowerCase()) >0){
...
public static void write(Node root, String outputfile){
    ...
    T.append("Student Number: ").append(root.getData().getStudentNumber()).append(" \t ").append("HomeDepartment: ").append(root.getData().getHomeDepartment()).append(" \t ").append("Program: ").append(root.getData().getProgram()).append(" \t ").append("Year: ").append(root.getData().getYear()).append("\t").append("Student LastName: ").append(root.getData().getStudentLastName()).append("\n");
    try{
...
public Node delete(Student student, Node p){
    ...
    if (student.getStudentLastName().toLowerCase().compareTo(p.getData().getStudentLastName().toLowerCase())< 0) { // remove from left child
